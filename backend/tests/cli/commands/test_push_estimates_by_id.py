...
class _StubRepo:
    ...
    async def get(self, *, subscription_id, year, month):
        stored_ids = {stored_id for stored_id, _ in self.stored}
        return object() if subscription_id in stored_ids else None
...
async def _drain(selector, repo, subscriptions, *, year=2026, month=5):
    ids = resolve_targets(selector, repo, subscriptions, year=year, month=month)
...
async def test_resolve_subscription_id_absent(repo, subscriptions, sub1):
    repo.stored = [sub1]
...
async def test_resolve_by_product(repo, subscriptions, sub1, sub2):
    repo.stored = [sub1, sub2]
    subscriptions.ids = ["SUB-2", "SUB-9"]  # SUB-9 is not in the DB
...
def test_uploads_anchored_on_last_month(mocker, runner, push_collaborators):
    ...
    anchor = today.replace(day=1) - dt.timedelta(days=1)
