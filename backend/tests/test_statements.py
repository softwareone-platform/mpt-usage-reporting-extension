...
def _context(mocker, product_ids=("PRD-1",), seller_id="", *, pages=None):
    ...
    service = api_client.billing.statements
    filtered = service.filter.return_value
    filtered.select.return_value.iterate.side_effect = [iter(page) for page in pages]
    return RunContext(
...
def _queries(ctx):
    filter_mock = ctx.api_client.billing.statements.filter
    return [str(call.args[0]) for call in filter_mock.call_args_list]


def test_select_filters_automated_products(mocker):
    ctx = _context(mocker, product_ids=("PRD-1", "PRD-2"))

    StatementSelector().select(ctx)  # act
...
def test_select_omits_seller_when_empty(mocker):
...
def test_select_includes_seller_when_set(mocker):
...
def test_select_runs_issued_and_cancelled_passes(mocker):
...
def test_select_renders_datetimes_unquoted(mocker):
    ctx = _context(mocker)
...
def test_select_filters_by_status_per_pass(mocker):
    ctx = _context(mocker)
...
def test_select_merges_duplicates_by_id(mocker):
    issued = [mocker.Mock(id="BILL-1")]
