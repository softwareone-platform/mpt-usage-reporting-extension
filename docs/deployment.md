...
| `MPT_DATABASE_URL` | - | `postgresql://postgres:postgres@postgres:5432/usage_reporting` | PostgreSQL connection string (libpq/psycopg format); required at runtime by every command that touches the store and by the schema migrations (both fail fast without it). In Azure with Entra auth, omit the password and set `sslmode=require`, e.g. `postgresql://<identity>@<server>.postgres.database.azure.com:5432/<db>?sslmode=require` |
| `MPT_DATABASE_ENTRA_AUTH` | `false` | `true` | When truthy (`1`/`true`/`yes`/`on`), the app acquires an Entra ID access token via the ambient workload-identity credential (`DefaultAzureCredential`) and supplies it as the PostgreSQL password at connect time. Leave unset/false for local development and password-based connections |
